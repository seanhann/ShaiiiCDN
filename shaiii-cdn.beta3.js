...
;var ShaiiiCDN = (function(){	
	...
	var Receiver = (function(){
		...
		function receiver(channel, chunkSize, security){
			...
			this.type = security ? security.type:null;
			this.bufferedSize = 0;
			this.buffer = [];
			this.success = false;
			this.init();
		...
		receiver.prototype.receive = function(data){
			...
			if(this.bufferedSize == this.size){
				...
				if(token == this.token){
					this.success = true;
					if(this.closure){
						var received = new Blob(this.buffer, {type: this.type});
	...
	var WebRTC = (function(){
		...
		webrtc.prototype.offer = function(fn){
			var that = this;
	  		this.connection.createOffer({voiceActivityDetection:false}).then(
	  			function(desc){
					log.write('offer set local');
		...
		webrtc.prototype.setRemoteDescription = function(remoteDesc){
		...
		webrtc.prototype.answer = function(remoteDesc, answerFun){
			...
			var that = this;
	  		this.setRemoteDescription(remoteDesc);
	  		this.connection.createAnswer( {voiceActivityDetection: false} ).then(
				function(desc){
					that.connection.setLocalDescription(desc);
					log.write('answer set local');
					answerFun(desc);
				},
				function(error){
					alert(error);
	...
	var Factory = function(){
		this.stunGoogle = { urls: 'stun:stun.l.google.com:19302' };
		this.stunMid = { urls: 'stun:mid.shaiii.com:3478', 'credential': 'hjxhlk@123', 'username': 'shaiii' };
		this.stunEast = { urls: 'stun:east.shaiii.com:3478', 'credential': 'hjxhlk@123', 'username': 'shaiii' };
		this.turn = { urls: 'turn:turn.shaiii.com:3478', 'credential': 'hjxhlk@123', 'username': 'shaiii' };
		//this.servers = { iceServers: [this.stunEast, this.stunMid, this.stunGoogle], iceCandidatePoolSize: 16};
		this.servers = { iceServers: [this.stunEast], iceCandidatePoolSize: 16};
	...
	var ShaiiiCDN = (function(){
		function cdn(option){
			...
			if(window.RTCPeerConnection){
				...
				this.db = new MyDB('ShaiiiCDN', 'caches');
		...
		cdn.prototype.bestGet = function(cache){
			...
			if(cache == 2){
				...
				if(imgs.length > 0) this.httpGet(imgs);
			}else if(cache == 1){
				for(src in this.resource){
				...
				if(imgs.length > 0) this.peerGet(imgs);
			}
		}
...
document.onreadystatechange = function () {
		...
		{
			if (allElements[i].getAttribute('shaiii-cdn') !== null)
			{
				matchingElements.push(allElements[i]);
			}
		}
                shaiiiCdn.get(matchingElements);
        }
};
